...
Usage:
    from orthon.services.fingerprint_service import FingerprintService
    ...
    def __init__(self, fingerprints_dir: Optional[str] = None):
        ...
        Args:
            fingerprints_dir: Directory containing fingerprint YAML files.
                             Defaults to ~/.orthon/fingerprints/
        """
        if fingerprints_dir:
            self.fingerprints_dir = Path(fingerprints_dir)
        else:
            self.fingerprints_dir = Path.home() / ".orthon" / "fingerprints"
