...
Natural language interface to ORTHON analysis.
...
class Concierge:
    """
    Natural language interface to ORTHON data.
    ...
    def __init__(self, data_dir: str):
        ...
        self.patterns = [
            ...
            (r"orthon signal", self._query_orthon_signal),
    ...
    def _query_healthiest(self, question: str) -> ConciergeResponse:
        ...
        WITH health AS (
            SELECT
                ...
                1 - (SUM(CASE WHEN dissipation_rate > 0.01 AND coherence < 0.5
                         AND state_velocity > 0.05 THEN 1 ELSE 0 END) * 1.0 / COUNT(*)) as orthon_score
            FROM physics
            GROUP BY entity_id
        )
        SELECT entity_id,
               ...
               ROUND(orthon_score, 3) as orthon_score,
               ROUND((avg_coherence * 0.4 + state_score * 0.3 + orthon_score * 0.3), 3) as health_score
        FROM health
        ...
        if data and 'entity_id' in data[0]:
            ...
            answer += f"- Orthon score: {data[0]['orthon_score']}"
        else:
    ...
    def _query_sickest(self, question: str) -> ConciergeResponse:
        ...
        WITH health AS (
            SELECT
                ...
                SUM(CASE WHEN dissipation_rate > 0.01 AND coherence < 0.5
                    AND state_velocity > 0.05 THEN 1 ELSE 0 END) as orthon_signal_count,
                COUNT(*) as total_obs
            FROM physics
            GROUP BY entity_id
        )
        SELECT entity_id,
               ...
               orthon_signal_count,
               ROUND(orthon_signal_count * 100.0 / total_obs, 1) as pct_critical
        FROM health
        ORDER BY max_state_distance DESC, orthon_signal_count DESC
        LIMIT 1
        """
        data = self._execute_sql(sql)
        ...
        if data and 'entity_id' in data[0]:
            ...
            answer += f"- Orthon signal: {data[0]['pct_critical']}% of time in critical state"
        else:
    ...
    def _query_orthon_signal(self, question: str) -> ConciergeResponse:
        sql = """
        SELECT
            entity_id,
            SUM(CASE WHEN dissipation_rate > 0.01 AND coherence < 0.5
        ...
        answer = "**Ørthon Signal Detection**\n\n"
        answer += "_The Ørthon signal indicates symplectic structure loss: dissipating + decoupling + diverging_\n\n"
        for d in data:
    ...
    def _query_first_failure(self, question: str) -> ConciergeResponse:
        ...
        WITH first_events AS (
            SELECT
                ...
                MIN(CASE WHEN dissipation_rate > 0.01 AND coherence < 0.5
                    AND state_velocity > 0.05 THEN I END) as first_orthon
            FROM physics
            GROUP BY entity_id
        )
        SELECT
        ...
        for d in data:
            ...
            if d['first_orthon']:
                answer += f"  - **Ørthon signal onset: I={d['first_orthon']}**\n"
            answer += "\n"
        ...
        if entity_match:
            ...
            WITH summary AS (
                SELECT
                    ...
                    SUM(CASE WHEN dissipation_rate > 0.01 AND coherence < 0.5
                        AND state_velocity > 0.05 THEN 1 ELSE 0 END) as orthon_count
                FROM physics
                WHERE entity_id = '{entity_id}'
                GROUP BY entity_id
            )
            ...
            if data:
                ...
                answer += f"**Ørthon Signal**: {d['orthon_count']} critical observations\n\n"
                ...
                if d['orthon_count'] > 0:
                    answer += f"⚠️ Ørthon signal active ({d['orthon_count']} times)\n"
            else:
    ...
    def _query_compare(self, question: str) -> ConciergeResponse:
        ...
        SELECT
            ...
            SUM(CASE WHEN dissipation_rate > 0.01 AND coherence < 0.5
                AND state_velocity > 0.05 THEN 1 ELSE 0 END) as orthon_signals
        FROM physics
        GROUP BY entity_id
        ORDER BY entity_id
        """
        ...
        answer += "| Entity | Coherence | State (σ) | Dimensions | Dissipation | Ørthon |\n"
        answer += "|--------|-----------|-----------|------------|-------------|--------|\n"
        for d in data:
            answer += f"| {d['entity_id']} | {d['coherence']} | {d['state_dist']} | {d['dimensions']} | {d['dissipation']} | {d['orthon_signals']} |\n"
    ...
    def _query_counts(self, question: str) -> ConciergeResponse:
        ...
        SELECT
            ...
            SUM(CASE WHEN dissipation_rate > 0.01 AND coherence < 0.5
                AND state_velocity > 0.05 THEN 1 ELSE 0 END) as orthon_signal_obs
        FROM physics
        """
        data = self._execute_sql(sql)
        ...
        answer += f"- {d['orthon_signal_obs']} Ørthon signal observations\n"
    ...
    def _query_general_summary(self, question: str) -> ConciergeResponse:
        ...
        SELECT
            ...
            SUM(CASE WHEN dissipation_rate > 0.01 AND coherence < 0.5
                AND state_velocity > 0.05 THEN 1 ELSE 0 END) as total_orthon_signals
        FROM physics
        """
        data = self._execute_sql(sql)
        ...
        answer += f"- Total Ørthon signals: {d['total_orthon_signals']}\n\n"
...
def ask_orthon(question: str, data_dir: str = ".") -> str:
    """
    Quick function to ask ORTHON a question.
