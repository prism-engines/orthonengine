...
WITH phase_with_lag AS (
    SELECT
        ...
        ps.phase_y,
        ps.phase_z,
        LAG(ps.phase_x) OVER w AS prev_phase_x,
        LAG(ps.phase_y) OVER w AS prev_phase_y,
        LAG(ps.phase_z) OVER w AS prev_phase_z
    FROM v_phase_space ps
    WINDOW w AS (PARTITION BY ps.signal_id ORDER BY ps.I)
),
phase_velocity_calc AS (
    SELECT
        signal_id,
        I,
        phase_x,
        ...
        phase_z,
        SQRT(
            COALESCE(POWER(phase_x - prev_phase_x, 2), 0) +
            COALESCE(POWER(phase_y - prev_phase_y, 2), 0) +
            COALESCE(POWER(phase_z - prev_phase_z, 2), 0)
        ) AS phase_velocity
    FROM phase_with_lag
),
phase_with_arc AS (
    SELECT
        ...
        phase_z,
        phase_velocity,
        SUM(phase_velocity) OVER (PARTITION BY signal_id ORDER BY I) AS arc_length
    FROM phase_velocity_calc
)
SELECT
    pa.signal_id,
    pa.I,
...
FROM phase_with_arc pa
LEFT JOIN v_signal_class sc USING (signal_id)
LEFT JOIN v_signal_typology st USING (signal_id)
LEFT JOIN v_regime_assignment ra ON pa.signal_id = ra.signal_id AND pa.I = ra.I
...
SELECT
    json_object(
        ...
        'alerts', (
            ...
            FROM (SELECT * FROM v_alerts ORDER BY severity DESC LIMIT 100)
        )
    ) AS manifold_data;
