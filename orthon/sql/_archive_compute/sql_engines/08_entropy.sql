...
SELECT
    ...
    bc.bin_count,
    tc.total_count,
    bc.bin_count::FLOAT / tc.total_count AS probability
FROM bin_counts bc
JOIN total_counts tc USING (signal_id);
...
total_counts AS (
    SELECT signal_id, SUM(bin_count) AS total_count
    FROM bin_counts GROUP BY signal_id
),
probs AS (
    SELECT
        bc.signal_id,
...
WITH max_I AS (
    SELECT signal_id, MAX(I) AS max_i FROM v_base GROUP BY signal_id
),
entropy_by_half AS (
    SELECT
        b.signal_id,
        CASE WHEN b.I < m.max_i / 2 THEN 'first_half' ELSE 'second_half' END AS half,
        b.y
    FROM v_base b
    JOIN max_I m USING (signal_id)
),
binned_halves AS (
    SELECT
        signal_id,
...
bin_counts AS (
    SELECT signal_id, half, bin, COUNT(*) AS cnt
    FROM binned_halves
    GROUP BY signal_id, half, bin
),
total_counts AS (
    SELECT signal_id, half, SUM(cnt) AS total
    FROM bin_counts GROUP BY signal_id, half
),
probs AS (
    SELECT
        bc.signal_id,
        bc.half,
        bc.cnt::FLOAT / tc.total AS probability
    FROM bin_counts bc
    JOIN total_counts tc USING (signal_id, half)
),
half_entropy AS (
    SELECT
        signal_id,
