...
baseline AS (
    SELECT
        ...
        COUNT(*) AS baseline_n
    FROM observations o
    JOIN time_bounds t ON o.entity_id = t.entity_id AND o.signal_id = t.signal_id
...
late_period AS (
    SELECT
        o.entity_id,
        o.signal_id,
        AVG(o.y) AS late_mean,
        STDDEV_POP(o.y) AS late_std,
        COUNT(*) AS late_n
    FROM observations o
    JOIN time_bounds t ON o.entity_id = t.entity_id AND o.signal_id = t.signal_id
...
SELECT
    ...
    END AS drift_status
FROM baseline b
JOIN late_period l ON b.entity_id = l.entity_id AND b.signal_id = l.signal_id
...
windowed AS (
    SELECT
        o.entity_id,
        o.signal_id,
        NTILE(10) OVER (PARTITION BY o.entity_id, o.signal_id ORDER BY o.I) AS window_id,
        o.y,
        o.I
    FROM observations o
),
...
SELECT
    ...
    END AS status
FROM window_stats w
JOIN baseline b ON w.entity_id = b.entity_id AND w.signal_id = b.signal_id
WHERE ABS((w.window_mean - b.baseline_mean) / NULLIF(b.baseline_std, 0)) > 0.5
  AND w.window_id > 2  -- Skip baseline windows
ORDER BY w.entity_id, w.signal_id, w.window_id;
...
windowed AS (
    SELECT
        o.entity_id,
        o.signal_id,
        NTILE(5) OVER (PARTITION BY o.entity_id, o.signal_id ORDER BY o.I) AS window_id,
        o.y
    FROM observations o
    JOIN time_bounds t ON o.entity_id = t.entity_id AND o.signal_id = t.signal_id
    WHERE o.I > t.baseline_end
...
drift_pattern AS (
    ...
    FROM window_stats w
    JOIN baseline b ON w.entity_id = b.entity_id AND w.signal_id = b.signal_id
    GROUP BY w.entity_id, w.signal_id, b.baseline_mean, b.baseline_std
)
...
baseline AS (
    SELECT o.entity_id, o.signal_id,
        VAR_POP(o.y) AS baseline_var,
        STDDEV_POP(o.y) AS baseline_std
    FROM observations o
    JOIN time_bounds t USING (entity_id, signal_id)
    WHERE o.I <= t.baseline_end
    GROUP BY o.entity_id, o.signal_id
),
...
FROM baseline b
JOIN late_period l USING (entity_id, signal_id)
...
post_baseline AS (
    SELECT o.signal_id, AVG(o.y) AS post_mean
    FROM observations o
    JOIN time_bounds t USING (signal_id)
    WHERE o.I > t.baseline_end
    GROUP BY o.signal_id
...
exceedances AS (
    SELECT
        ...
        b.mu,
        b.sigma,
        (o.y - b.mu) / NULLIF(b.sigma, 0) AS z_score
    FROM observations o
    JOIN baseline b USING (signal_id)
    JOIN time_bounds t USING (signal_id)
    WHERE o.I > t.baseline_end
      AND ABS((o.y - b.mu) / NULLIF(b.sigma, 0)) > 2
)
...
window_drift AS (
    SELECT
        w.signal_id,
        w.window_id,
        (AVG(w.y) - b.mu) / NULLIF(b.sigma, 0) AS drift_sigma
    FROM windowed w
    JOIN baseline b USING (signal_id)
