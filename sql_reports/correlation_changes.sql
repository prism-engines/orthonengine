...
window_corr AS (
    ...
    JOIN windowed b
        ...
        AND a.signal_id < b.signal_id
    GROUP BY a.entity_id, a.window_id, a.signal_id, b.signal_id
)
...
baseline_wide AS (
    SELECT o.entity_id, o.I, o.signal_id, o.y
    FROM observations o
    JOIN time_bounds t ON o.entity_id = t.entity_id
    WHERE o.I <= t.baseline_end
),
...
baseline_corr AS (
    SELECT
        a.entity_id, a.signal_id AS signal_a, b.signal_id AS signal_b,
        CORR(a.y, b.y) AS correlation
    FROM baseline_wide a
    JOIN baseline_wide b ON a.entity_id = b.entity_id AND a.I = b.I AND a.signal_id < b.signal_id
    GROUP BY a.entity_id, a.signal_id, b.signal_id
),
...
late_corr AS (
    SELECT
        ...
        CORR(a.y, b.y) AS correlation
    FROM late_wide a
    JOIN late_wide b ON a.entity_id = b.entity_id AND a.I = b.I AND a.signal_id < b.signal_id
    GROUP BY a.entity_id, a.signal_id, b.signal_id
)
...
FROM baseline_corr b
JOIN late_corr l USING (entity_id, signal_a, signal_b)
