...
--
-- Usage: Run against observations table with columns [entity_id, signal_id, I, y]
-- ============================================================================
...
signal_lags AS (
    SELECT
        ...
        LAG(a.y, 2) OVER wa AS y_a_lag2,
        LAG(a.y, 5) OVER wa AS y_a_lag5,
        LAG(a.y, 10) OVER wa AS y_a_lag10,
        LEAD(a.y, 1) OVER wa AS y_a_lead1,
        LEAD(a.y, 2) OVER wa AS y_a_lead2,
        LEAD(a.y, 5) OVER wa AS y_a_lead5,
        LEAD(a.y, 10) OVER wa AS y_a_lead10
    FROM observations a
    JOIN observations b
...
deviations AS (
    SELECT
        ...
        o.signal_id,
        o.I,
        (o.y - b.baseline_mean) / NULLIF(b.baseline_std, 0) AS z_score
    FROM observations o
    JOIN baseline_stats b ON o.entity_id = b.entity_id AND o.signal_id = b.signal_id
    JOIN time_bounds t ON o.entity_id = t.entity_id
    WHERE o.I > t.baseline_end
),
...
SELECT
    ...
    RANK() OVER (PARTITION BY entity_id ORDER BY first_exceed_2sigma NULLS LAST) AS response_order,
    CASE
        WHEN RANK() OVER (PARTITION BY entity_id ORDER BY first_exceed_2sigma NULLS LAST) <= 3 THEN 'FIRST_RESPONDER'
        WHEN RANK() OVER (PARTITION BY entity_id ORDER BY first_exceed_2sigma NULLS LAST) <= 10 THEN 'EARLY_RESPONDER'
        WHEN first_exceed_2sigma IS NOT NULL THEN 'LATE_RESPONDER'
        ELSE 'NO_DEVIATION'
    END AS response_class
FROM first_deviation
ORDER BY entity_id, first_exceed_2sigma NULLS LAST;
...
baseline_stats AS (
    SELECT o.entity_id, o.signal_id,
        AVG(o.y) AS baseline_mean, STDDEV_POP(o.y) AS baseline_std
...
first_deviation AS (
    SELECT
        o.entity_id,
        o.signal_id,
        MIN(CASE WHEN ABS((o.y - b.baseline_mean) / NULLIF(b.baseline_std, 0)) > 2.0 THEN o.I END) AS first_dev_time
    FROM observations o
    JOIN baseline_stats b USING (entity_id, signal_id)
    JOIN time_bounds t USING (entity_id)
    WHERE o.I > t.baseline_end
    GROUP BY o.entity_id, o.signal_id
),
...
ORDER BY entity_id, source_dev_time, propagation_delay;
...
downstream_count AS (
    ...
    GROUP BY a.entity_id, a.signal_id, a.first_dev_time
)
...
SELECT
    ...
    RANK() OVER (PARTITION BY entity_id ORDER BY first_dev_time) AS deviation_order,
    CASE
        WHEN RANK() OVER (PARTITION BY entity_id ORDER BY first_dev_time) = 1 THEN 'ROOT_CAUSE_CANDIDATE'
        WHEN RANK() OVER (PARTITION BY entity_id ORDER BY first_dev_time) <= 3 THEN 'EARLY_INDICATOR'
        ELSE 'DOWNSTREAM'
    END AS role
FROM downstream_count
ORDER BY entity_id, first_dev_time;
...
windowed AS (
    SELECT
        entity_id,
        signal_id,
        NTILE(5) OVER (PARTITION BY entity_id, signal_id ORDER BY I) AS window_id,
        I,
        y
    FROM observations
),
...
window_pairs AS (
    ...
    JOIN windowed b
        ...
        AND a.signal_id < b.signal_id
    WINDOW w AS (PARTITION BY a.entity_id, a.signal_id, b.signal_id, a.window_id ORDER BY a.I)
),
...
window_lead_lag AS (
    ...
    GROUP BY entity_id, window_id, signal_a, signal_b
)
...
ORDER BY entity_id, ABS(MAX(sync_corr) - MIN(sync_corr)) DESC;
...
SELECT
    ...
    (SELECT signal_id FROM first_deviation f2
     WHERE f2.entity_id = first_deviation.entity_id
