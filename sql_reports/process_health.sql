...
signal_health AS (
    ...
    FROM baseline b
    JOIN current_state c ON b.entity_id = c.entity_id AND b.signal_id = c.signal_id
)
...
window_means AS (
    SELECT entity_id, signal_id, window_id, AVG(y) AS window_mean
    FROM windowed
...
baseline AS (
    SELECT o.signal_id, AVG(o.y) AS mu, STDDEV_POP(o.y) AS sigma
...
anomalies AS (
    SELECT
        o.signal_id,
        o.I,
        ABS((o.y - b.mu) / NULLIF(b.sigma, 0)) AS z_score
    FROM observations o
...
current_state AS (
    SELECT o.signal_id, AVG(o.y) AS current_mean, STDDEV_POP(o.y) AS current_std
    FROM observations o, time_bounds t
    WHERE o.I > t.baseline_end
    GROUP BY o.signal_id
),
