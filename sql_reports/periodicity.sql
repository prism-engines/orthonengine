...
centered AS (
    SELECT
        ...
        signal_id,
        I,
        y - AVG(y) OVER (PARTITION BY entity_id, signal_id) AS y_centered
    FROM observations
),
...
FROM crossings c
JOIN signal_range r USING (entity_id, signal_id)
GROUP BY c.entity_id, c.signal_id, r.n_points, r.i_min, r.i_max
...
centered AS (
    SELECT
        ...
        y,
        y - AVG(y) OVER (PARTITION BY entity_id, signal_id) AS y_centered,
        STDDEV_POP(y) OVER (PARTITION BY entity_id, signal_id) AS y_std
    FROM observations
),
...
FROM extrema_stats
ORDER BY entity_id, avg_amplitude / NULLIF(signal_std, 0) DESC;
...
peaks AS (
    SELECT
        ...
        signal_id,
        I,
        ROW_NUMBER() OVER (PARTITION BY entity_id, signal_id ORDER BY I) AS peak_num
    FROM centered
    WHERE y_centered > LAG(y_centered) OVER (PARTITION BY entity_id, signal_id ORDER BY I)
      AND y_centered > LEAD(y_centered) OVER (PARTITION BY entity_id, signal_id ORDER BY I)
),
...
peak_intervals AS (
    SELECT
        entity_id,
        signal_id,
        I - LAG(I) OVER (PARTITION BY entity_id, signal_id ORDER BY peak_num) AS interval
    FROM peaks
)
...
GROUP BY entity_id, signal_id
HAVING COUNT(*) >= 3
ORDER BY entity_id, period_variability;
...
FROM correlations
ORDER BY entity_id, acf_lag1 DESC;
...
ORDER BY entity_id, MAX(rate) - MIN(rate) DESC;
...
centered AS (
    SELECT
        ...
        I,
        y,
        AVG(y) OVER (PARTITION BY entity_id, signal_id) AS mean_val,
        STDDEV_POP(y) OVER (PARTITION BY entity_id, signal_id) AS std_val,
        y - AVG(y) OVER (PARTITION BY entity_id, signal_id) AS deviation
    FROM observations
),
...
FROM hunting_stats
ORDER BY entity_id, 1.0 * n_reversals / n_points DESC;
