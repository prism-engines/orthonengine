...
windowed AS (
    SELECT
        entity_id,
        signal_id,
        NTILE(20) OVER (PARTITION BY entity_id, signal_id ORDER BY I) AS window_id,
        I,
        y
    FROM observations
),
...
window_stats AS (
    SELECT
        entity_id,
        window_id,
        AVG(y) AS overall_mean,
        STDDEV_POP(y) AS overall_std
    FROM windowed
...
regime_boundaries AS (
    ...
    FROM window_stats
    WINDOW w AS (PARTITION BY entity_id ORDER BY window_id)
)
...
SELECT
    ...
    SUM(CASE WHEN mean_change_z > 1.5 OR std_ratio > 1.5 OR std_ratio < 0.67 THEN 1 ELSE 0 END)
        OVER (PARTITION BY entity_id ORDER BY window_id) AS regime_id
...
window_stats AS (
    SELECT entity_id, window_id,
        AVG(y) AS overall_mean, STDDEV_POP(y) AS overall_std
    FROM windowed
    GROUP BY entity_id, window_id
),
...
regime_boundaries AS (
    SELECT
        entity_id, window_id, overall_mean, overall_std,
        ABS(overall_mean - LAG(overall_mean) OVER w) / NULLIF(overall_std, 0) AS mean_change_z,
        overall_std / NULLIF(LAG(overall_std) OVER w, 0) AS std_ratio
    FROM window_stats
    WINDOW w AS (PARTITION BY entity_id ORDER BY window_id)
...
SELECT
    ...
    CASE
        WHEN AVG(overall_std) > 1.5 * (SELECT AVG(overall_std) FROM regime_windows WHERE entity_id = r.entity_id) THEN 'HIGH_VARIABILITY'
        WHEN AVG(overall_std) < 0.5 * (SELECT AVG(overall_std) FROM regime_windows WHERE entity_id = r.entity_id) THEN 'LOW_VARIABILITY'
        ELSE 'NORMAL_VARIABILITY'
    END AS variability_class
FROM regime_windows r
GROUP BY entity_id, regime_id
ORDER BY entity_id, regime_id;
...
windowed AS (
    SELECT
        entity_id,
        signal_id,
        NTILE(50) OVER (PARTITION BY entity_id, signal_id ORDER BY I) AS window_id,
        y
    FROM observations
),
...
transients AS (
    ...
    FROM rates r
    JOIN rate_stats s USING (entity_id, signal_id)
)
...
FROM transients
WHERE ABS(rate_z) > 2
ORDER BY entity_id, I, ABS(rate_z) DESC;
...
normalized AS (
    SELECT
        ...
        overall_mean,
        overall_std,
        (overall_mean - AVG(overall_mean) OVER (PARTITION BY entity_id)) /
            NULLIF(STDDEV(overall_mean) OVER (PARTITION BY entity_id), 0) AS norm_mean,
        (overall_std - AVG(overall_std) OVER (PARTITION BY entity_id)) /
            NULLIF(STDDEV(overall_std) OVER (PARTITION BY entity_id), 0) AS norm_std
...
windowed AS (
    SELECT entity_id, signal_id,
        NTILE(20) OVER (PARTITION BY entity_id, signal_id ORDER BY I) AS window_id,
        MIN(I) OVER (PARTITION BY entity_id, signal_id,
            NTILE(20) OVER (PARTITION BY entity_id, signal_id ORDER BY I)) AS window_start,
        MAX(I) OVER (PARTITION BY entity_id, signal_id,
            NTILE(20) OVER (PARTITION BY entity_id, signal_id ORDER BY I)) AS window_end,
        y
    FROM observations
),

window_stats AS (
    SELECT entity_id, window_id, MIN(window_start) AS t_start, MAX(window_end) AS t_end,
        AVG(y) AS overall_mean, STDDEV_POP(y) AS overall_std
    FROM windowed
    GROUP BY entity_id, window_id
),
